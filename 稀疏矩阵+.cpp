	...
	Sparse operator + (Sparse & sparse2) const {
		...
		for (int j = 0; j < len2; ++ j) { 
			...
			for (int i = 0; i < len1; ++ i) {
				if(sparse3.entrys[i].column == sparse2.entrys[j].column && sparse3.entrys[i].row == sparse2.entrys[j].row) {
					flag = 1;
					sparse3.entrys[i].value += sparse2.entrys[j].value; //第二种情况 
				} 
			}
			if(flag == 0) sparse3.entrys.push_back(sparse2.entrys[j]); //第一种情况 
		}
		
		for (int i = 0; i < sparse3.entrys.size(); ++ i) { //第三种情况，注意sparse3的size已经发生了变化 
			if(sparse3.entrys[i].value == 0) sparse3.entrys.erase(sparse3.entrys.begin()+i); //注意vector的erase函数的用法，必须有个vec.begin()，注意函数参数类型为迭代器 
		} 
